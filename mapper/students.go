...
func MapStudentFullProfileRowToDomain(row db.GetStudentFullProfileRow) domain.Student {
	return domain.Student{
		TenantUUIDModel: domain.TenantUUIDModel{
			InstituteID: row.InstituteID,
			BaseUUIDModel: domain.BaseUUIDModel{
		...
		// Extended fields to be added to domain model or handled separately
		// For now, we return the base student struct.
		// If domain.Student has fields for ClassName etc, we map them here.
	}
}
...
func MapUpdateStudentParams(s domain.Student) db.UpdateStudentParams {
	return db.UpdateStudentParams{
		ID:             s.ID,
		InstituteID:    s.InstituteID,
		FirstName:      helper.ToNullString(s.FirstName),
		LastName:       helper.ToNullString(helper.StrOrEmpty(s.LastName)),
		CurrentClassID: helper.ToNullUUID(helper.DerefUUID(s.CurrentClassID)),
		UpdatedBy:      helper.ToNullUUID(helper.DerefUUID(s.UpdatedBy)),
	}
}
